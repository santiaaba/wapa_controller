...
void json_site(char *data, int *size, T_site *site){
	...
	sprintf(data,"{\"name\":\"%s\"",data,site_get_name(site));
	sprintf(data,"%s,\"site_id\":\"\"",data,site_get_id(site));
	sprintf(data,"%s,\"user_id\":\"\"",data,site_get_userid(site));
	sprintf(data,"%s,\"suscrip_id\":\"\"",data,site_get_susc(site));
	sprintf(data,"%s,\"status\":\"\"",data,site_get_status(site));
	sprintf(data,"%s,\"urls\": [",data);
	...
	while(list_alias_eol(site_get_alias(site))){
		...
		sprintf(data,"%s {\"%lu\":\"%s\",\"url\":\"%s\"},",data,alias_get_id(alias),alias_get_name(alias));
		list_alias_next(site_get_alias(site));
	}
	data[strlen(data)-1] = ']';
	sprintf(data,"%s,\"workers\": [",data);
	...
	while(list_worker_eol(site_get_workers(site))){
		...
		sprintf(data,"%s {\"name\":\"%s\"},",data,worker_get_name(worker));
